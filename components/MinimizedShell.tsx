...
export default function MinimizedShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (isMinimized) {
      ...
      const portfolioRevealLines = document.querySelectorAll<HTMLElement>('.portfolio .reveal-inner');
      if (portfolioRevealLines.length) {
        gsap.set(portfolioRevealLines, { clearProps: 'y,transform' });
      }
